...
void CLibTrilateration::test_accuracy_tri_num_pnt_ref4()
{
	...
	//// DO NOT EDIT BELOW
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	const int width_mat_pnt_ref = 3;	// (x,y,z)
	const int num_pnt_ref = 4;			// set num of reference pnts being measured


	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//// set input (please edit this)
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	const bool	b_add_noise = true;

	const	int		totnum_iter		= 1000;		// totnum of iter / noise

	const	double  z_mover_gt_s	= 10.0;
	const	double  z_mover_gt_e	= 50.0;
	const	double  inc_z_mover_gt	= 10.0;

	const	double  y_mover_gt_s	= -5.0;
	const	double  y_mover_gt_e	= 15.0;
	const	double  inc_y_mover_gt	= 1.0;

	const	double  x_mover_gt_s	= -5.0;
	const	double  x_mover_gt_e	= 15.0;
	const	double  inc_x_mover_gt	= 1.0;
	...
	mat_pnt_ref[1 * width_mat_pnt_ref + 0] = 10.0;		// row1: TD101
	...
	mat_pnt_ref[2 * width_mat_pnt_ref + 1] = 10.0;
	mat_pnt_ref[2 * width_mat_pnt_ref + 2] = 1.2;

	mat_pnt_ref[3 * width_mat_pnt_ref + 0] = 10.0;		// row3: TD103
	mat_pnt_ref[3 * width_mat_pnt_ref + 1] = 10.0;
	...
	double x_mover_gt, y_mover_gt, z_mover_gt;
	double range_gt[4];
	double range_in[4];
	...
	for (i = 0; i < 11; i++)
	{
		vec_mag_noise[i] = ((double)i)*0.1;		// 0.1: 0.1m
	}


	for (z_mover_gt = z_mover_gt_s; z_mover_gt <= z_mover_gt_e; z_mover_gt += inc_z_mover_gt)
	{
		...
		for (idx_noise = 0; idx_noise < 11; idx_noise++)
		{
			...
			printf("  [%d] - mag_noise:(%.2lf)...\n", idx_noise, mag_noise);

			for (idx_iter = 0; idx_iter < totnum_iter; idx_iter++)
			{
				//if ((idx_iter % 1000) == 0) { printf("        idx_iter: [%d]\n", idx_iter); }


				////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				for (y_mover_gt = y_mover_gt_s; y_mover_gt <= y_mover_gt_e; y_mover_gt += inc_y_mover_gt)
				{
					for (x_mover_gt = x_mover_gt_s; x_mover_gt <= x_mover_gt_e; x_mover_gt += inc_x_mover_gt)
					{
