...
% show raw orientation and delta orientation
...
for idx_data_packet = 1:num_data_packet,    
    ...
    if data_type == DATA_IMU,        
        ...
        if b_first_imu == true,
            mat_body_rot0 = mat_body_rot;
            b_first_imu = false;
        end
        
        mat_body_rot_delta = mat_body_rot0'*mat_body_rot;
                
        
        %%%% visualize raw orientation
        if 0,
            if mod(cnt_imu, 20) == 0,
                format_fname_img_res = './screenshot_saved/img_ori_raw_%d.png';

                format_str_title = 'Raw ori, Time: %fs';
                str_title = sprintf(format_str_title, time_this);

                visualize_simple_this2(cnt_imu, str_title, format_fname_img_res, mat_body_rot, mat_body_trans);
            end
        end

        
        %%%% visualize delta orientation
        if 1,
            if mod(cnt_imu, 20) == 0,
                format_fname_img_res = './screenshot_saved/img_ori_delta_%d.png';

                format_str_title = 'Delta ori, Time: %fs';
                str_title = sprintf(format_str_title, time_this);

                visualize_simple_this2(cnt_imu, str_title, format_fname_img_res, mat_body_rot_delta, mat_body_trans);
            end
        end
